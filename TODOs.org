...
**** NOW Migrate registry/global APIs to datastructure/local APIs :architecture:
     ...
     - [ ] lib/dispatcher.fnl (still imports singleton instances from :events and :event_sources; side-effects on require; needs register-dispatcher! function)
     - [X] source-types-register (lib/source-registry.fnl)
     - [X] source-instances-register (lib/source-registry.fnl)

**** NEXT Remove external requires from lib/ modules              :architecture:
