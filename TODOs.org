...
** Hierarchy
*** NOW derive should check for cycles                                :feature:
    - Prevent A derives B derives A situations

*** NEXT Cache ancestor lookups                                          :perf:
    - Currently recalculated each time
    - Invalidate cache on derive/underive

*** LATER [#B] Make hierarchies non-global                        :architecture:
...
** Events
*** NEXT Define all events in one file with proper domain-based event-kinds :refactor:
...
** Subscriptions
*** NEXT Separate subscription into its own module                    :refactor:
...
** API/Naming
*** NOW Rename functions to define-event/behavior/etc                 :refactor:

*** NOW Rename registry maps to *-register                           :refactor:

*** NOW Validate dependencies when defining things                    :feature:
    - When behavior definition mentions event-names, validate they exist
    - Error early if dependencies are missing
